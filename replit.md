...
- **Custom Domains**: Tenants connect first-level domains (e.g., myshop.kz) via Cloudflare CNAME proxy. CNAME target is the platform's published domain (PLATFORM_DOMAIN env var = `botfactory.kz`). With Cloudflare Proxy (orange cloud) enabled, Cloudflare uses the CNAME target for SNI routing (so Replit routes to the correct app) while preserving the original Host header (so our middleware detects the custom domain). This allows self-service custom domains without manual Replit deployment config per tenant. Server middleware detects custom domains via Host header and serves tenant catalog at root path. Domain verification via POST /api/tenant/domain-verify uses dual approach: HTTP probe (GET /.well-known/catalog-verify/:tenantId) and DNS resolution check (CNAME or A-record via Cloudflare proxy). GET /api/platform-domain returns the target CNAME domain. SSL mode in Cloudflare should be set to "Full". Domain normalization helper strips protocol/www/paths/ports consistently. Schema fields: customDomain, domainVerified. Client-side uses `useCatalogSlug` hook (client/src/hooks/useCatalogSlug.ts) and `basePath` prop pattern for clean URLs on custom domains.
