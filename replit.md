...
-   **AI-РОП Module Structure**:
    ...
    -   **Audience**: Manages auto-syncing contacts via WAHA/Meta, provides contact filtering, and allows segment management. WAHA sync has a local DB fallback: if the WAHA API returns no chats (common with NOWEB engine), contacts are populated from `ai_conversations`, `orders`, and `messaging_messages` tables. Phone normalization strips WhatsApp suffixes and non-digit characters consistently across all sync providers. Contacts get auto-tags: `has_order`, `paid`, `successful`, `active`, `handoff`.
