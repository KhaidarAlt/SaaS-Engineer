...
-   **Scraper Date Depth & Product Limit (Task #5)**: Demo pipeline uses a 1-month cutoff. On onboarding form (Telegram source), user selects depth 1–6 months (default 3). After payment confirmation, `runFullScrape` scrapes up to tenant's `catalogProductLimit` (default 200). Tenants table has `catalogProductLimit integer default 200`. Sessions have `scrapeDepthMonths integer`. `scrapePackages` table stores upgrade requests (+1000 @ 6990₸ or +5000 @ 12990₸). Dashboard shows a `CatalogLimitCard` (only for magic-import tenants) with a progress bar and upgrade modal with Kaspi payment flow. Admin panel shows pending package requests with a one-click confirm button that automatically increases `catalogProductLimit`.
